...
func TestServeSegment_MismatchHashError(t *testing.T) {
	...
	creds, err := genSegCreds(s, &stream.HLSSegment{}, nil)
	require.Nil(t, err)
...
func TestServeSegment_TranscodeSegError(t *testing.T) {
	...
	creds, err := genSegCreds(s, seg, nil)
	require.Nil(err)
...
func TestGenSegCreds_FullProfiles(t *testing.T) {
	...
	data, err := genSegCreds(s, seg, nil)
	assert.Nil(err)
...
func TestServeSegment_SaveDataFormat(t *testing.T) {
	...
	creds, err := genSegCreds(sess, &stream.HLSSegment{}, nil)
	assert.Nil(err)
	...
	creds, _ = genSegCreds(sess, &stream.HLSSegment{}, nil)
	os = &stubOSSession{} // reset for simplicity
	tRes.OS = os
	headers = map[string]string{segmentHeader: creds}
	resp = httpPostResp(handler, bytes.NewReader([]byte("")), headers)
...
func TestSubmitSegment_GenSegCredsError(t *testing.T) {
	...
	_, err := SubmitSegment(context.TODO(), s, &stream.HLSSegment{}, nil, 0, true)
...
func TestSubmitSegment_Timeout(t *testing.T) {
	...
	_, err = SubmitSegment(context.TODO(), sess, seg, nil, 0, true)
	assert.Nil(err)
	...
	lock.Unlock()
	_, err = SubmitSegment(context.Background(), sess, seg, nil, 0, true)
	assert.NotNil(err)
...
func TestSubmitSegment_Success(t *testing.T) {
	...
	tdata, err := SubmitSegment(context.TODO(), s, noNameSeg, nil, 0, true)
	...
	SubmitSegment(context.TODO(), s, seg, nil, 0, true)
