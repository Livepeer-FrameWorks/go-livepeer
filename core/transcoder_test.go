...
func TestProfilesToTranscodeOptions(t *testing.T) {
	...
	makeMeta := func(p []ffmpeg.VideoProfile) *SegTranscodingMetadata {
	...
	opts := profilesToTranscodeOptions(workDir, ffmpeg.Software, makeMeta(profiles))
	...
	opts = profilesToTranscodeOptions(workDir, ffmpeg.Nvidia, makeMeta(profiles))
	assert.Equal(2, len(opts))
