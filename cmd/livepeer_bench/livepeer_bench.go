...
func main() {
	...
	qsv := flag.String("qsv", "", "Comma-separated list of Intel QSV device paths (or \"all\" for all available devices)")
	videotoolbox := flag.String("videotoolbox", "", "Enable Apple VideoToolbox hardware acceleration (use \"all\" on macOS)")
	...
	if *qsv != "" {
		var err error
		accel = ffmpeg.QSV
		devices, err = common.ParseAccelDevices(*qsv, accel)
		if err != nil {
			glog.Exitf("Error while parsing '-qsv %v' flag: %v", *qsv, err)
		}
	}
	...
	if *videotoolbox != "" {
		...
		devices, err = common.ParseAccelDevices(*videotoolbox, accel)
		if err != nil {
			glog.Exitf("Error while parsing '-videotoolbox %v' flag: %v", *videotoolbox, err)
		}
	}
	...
	if accel == ffmpeg.QSV && len(devices) > 0 {
		data = append(data, []string{"QSV Devices", fmt.Sprintf("%v", strings.Join(devices, ","))})
	}

	if accel == ffmpeg.Videotoolbox {
		data = append(data, []string{"VideoToolbox", "enabled"})
	}
